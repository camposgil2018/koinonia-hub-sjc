    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Calendário completo de cultos, reuniões e eventos.</p>
        </div>
            ...
            {grid.map((c, i) => {
              ...
              return (
                ...
                >
                  {c.date && (
                    <>
                      <div className={cn("text-[11px] font-medium", isToday ? "text-gold" : "text-muted-foreground")}>
                        {c.date.slice(8, 10)}
                      </div>
                      <div className="mt-1 space-y-0.5">
                        {ev.slice(0, 2).map((e) => (
                          <button
                            ...
                            className={cn("w-full truncate rounded px-1 py-0.5 text-[10px] sm:text-[11px] font-medium text-left", categoryColor[e.category] ?? "bg-secondary text-secondary-foreground")}
                          >
                            {e.title}
                          </button>
                        ))}
      ...
      <div className="flex flex-wrap gap-2">
        {CATALOG.EVENT_CATEGORIES.map((c) => (
          <Badge key={c} className={cn("font-normal", categoryColor[c])}>{c}</Badge>
        ))}
      </div>
      ...
      <Dialog open={!!selected} onOpenChange={(o) => !o && setSelected(null)}>
        <DialogContent>
          {selected && (
            <>
              <DialogHeader>
                <Badge className={cn("w-fit mb-2", categoryColor[selected.category])}>{selected.category}</Badge>
                <DialogTitle className="font-display text-2xl">{selected.title}</DialogTitle>
              </DialogHeader>
              <div className="space-y-3 text-sm">
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Clock className="h-4 w-4" />
                  {new Date(selected.date + "T12:00:00").toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "long", year: "numeric" })} • {selected.time}
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
...
function NewEventDialog() {
  ...
  const [form, setForm] = useState<Omit<ChurchEvent, "id">>({
    title: "", date: "", time: "19:00", location: "", description: "", category: "Culto",
  });

  const save = () => {
    if (!form.title || !form.date || !form.location) { toast.error("Preencha título, data e local"); return; }
    store.set((s) => ({ ...s, events: [...s.events, { ...form, id: uid() }] }));
    toast.success("Evento adicionado à agenda");
    setForm({ title: "", date: "", time: "19:00", location: "", description: "", category: "Culto" });
    setOpen(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="gap-2"><Plus className="h-4 w-4" />Adicionar evento</Button>
      </DialogTrigger>
      <DialogContent>
        ...
        <div className="grid gap-3">
          ...
          <div>
            ...
            <Select value={form.category} onValueChange={(v) => setForm({ ...form, category: v })}>
              ...
              <SelectContent>{CATALOG.EVENT_CATEGORIES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
            </Select>
          </div>
