...
const VERSES = [
  ...
  { ref: "Atos 2:42", text: "E perseveravam na doutrina dos apóstolos, e na comunhão, e no partir do pão, e nas orações." },
  { ref: "Hebreus 10:25", text: "Não deixemos de congregar-nos, como é costume de alguns, antes admoestemo-nos uns aos outros." },
  { ref: "Romanos 12:5", text: "Assim nós, embora muitos, somos um só corpo em Cristo e individualmente membros uns dos outros." },
];
    ...
    <div className="space-y-6">
            ...
            {myNext ? (
              <div className="space-y-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <Badge className="bg-success text-success-foreground hover:bg-success">Confirmado</Badge>
                </div>
                  ...
                  {myNext.assignments
                    ...
                    .map((a, i) => (
                      <div key={i} className="rounded-lg border border-border bg-muted/30 p-3">
                        <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{a.ministry}</div>
                        <div className="text-sm font-medium mt-0.5">{a.role}</div>
                      </div>
                    ))}
                </div>
                <button onClick={() => goTo("schedules")} className="text-sm text-primary hover:underline">
              ...
              <div className="text-sm text-muted-foreground py-4">
                ...
                <br />
                <button onClick={() => goTo("schedules")} className="text-primary hover:underline mt-2 inline-block">
            ...
            {upcomingEvents.map((e) => (
              <div key={e.id} className="flex gap-3">
                <div className="flex flex-col items-center justify-center rounded-md bg-secondary text-secondary-foreground w-12 h-12 shrink-0">
                  <div className="text-[10px] uppercase">{new Date(e.date + "T12:00:00").toLocaleDateString("pt-BR", { month: "short" }).replace(".", "")}</div>
      ...
      <Card>
        <CardHeader className="pb-3 flex-row items-center justify-between space-y-0">
          ...
          <button onClick={() => goTo("notices")} className="text-xs text-primary hover:underline">Ver mural →</button>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-3">
          {recentNotices.map((n) => (
            <div key={n.id} className="rounded-lg border border-border p-4 hover:border-primary/40 transition-colors">
              <div className="flex items-center gap-2 mb-2">
                {n.pinned && <Pin className="h-3 w-3 text-gold" />}
                <Badge variant="secondary" className="text-[10px] uppercase tracking-wide">{n.category}</Badge>
              </div>
              <div className="font-medium text-sm">{n.title}</div>
              <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{n.content}</p>
              <div className="text-[11px] text-muted-foreground mt-3">{n.author} • {new Date(n.date + "T12:00:00").toLocaleDateString("pt-BR")}</div>
            </div>
          ))}
        </CardContent>
      </Card>
