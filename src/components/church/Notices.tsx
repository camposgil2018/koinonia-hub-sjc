    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Comunicados importantes da nossa comunidade.</p>
        </div>
...
function NoticeCard({ notice, canEdit }: { notice: Notice; canEdit: boolean }) {
  const togglePin = () => {
    store.set((s) => ({ ...s, notices: s.notices.map((n) => n.id === notice.id ? { ...n, pinned: !n.pinned } : n) }));
  ...
  return (
    <Card className={cn("relative overflow-hidden transition-all hover:shadow-md", notice.pinned && "ring-1 ring-gold/50")}>
      {notice.pinned && (
      ...
      <CardContent className="p-5 space-y-3">
        <Badge className={cn("font-normal", categoryStyles[notice.category] ?? "bg-secondary")}>{notice.category}</Badge>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border">
          ...
          {canEdit && (
            <div className="flex items-center gap-1">
              ...
              <NoticeDialog mode="edit" notice={notice} trigger={<Button variant="ghost" size="icon" className="h-7 w-7"><Pencil className="h-3.5 w-3.5" /></Button>} />
...
function NoticeDialog({
  mode, notice, trigger,
}: { mode: "create" | "edit"; notice?: Notice; trigger?: React.ReactNode }) {
  const me = useStore((s) => s.users.find((u) => u.id === s.currentUserId)!);
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState<Omit<Notice, "id">>(() => notice ?? {
    title: "", category: "Geral", content: "", date: new Date().toISOString().slice(0, 10), author: me.name, pinned: false,
  });
  ...
  const save = () => {
    ...
    if (mode === "create") {
      ...
      store.set((s) => ({ ...s, notices: s.notices.map((n) => n.id === notice!.id ? { ...n, ...form } : n) }));
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {trigger ?? <Button className="gap-2"><Plus className="h-4 w-4" />Novo aviso</Button>}
      </DialogTrigger>
      <DialogContent>
        <DialogHeader><DialogTitle>{mode === "create" ? "Novo aviso" : "Editar aviso"}</DialogTitle></DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Select value={form.category} onValueChange={(v) => setForm({ ...form, category: v })}>
                ...
                <SelectContent>{CATALOG.NOTICE_CATEGORIES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
              </Select>
            </div>
